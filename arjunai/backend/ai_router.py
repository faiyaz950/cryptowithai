...
def _load_gemini_api_keys() -> list[str]:
    ...
    for name in ("GEMINI_API_KEY", "GEMINI_API_KEY_2", "GEMINI_API_KEY_3"):
        ...
        if not raw or raw.lower().startswith("your_") or raw in seen:
            continue
        seen.add(raw)
        keys.append(raw)
...
MODEL_DESCRIPTIONS = {
    "auto": "Gemini first — same Google model, search on if quota allows",
...
class ArjunAI:
    def __init__(self):
        ...
        self.groq_model_ids = [groq_model]
    ...
    def _build_agent_list(self, preferred_model: Optional[str], user_type: str, streaming: bool) -> list:
        ...
        if model in ("auto", "gemini"):
            return [(n, fn) for n, fn in configured if n == "gemini"]
            ...
            for model_id in self.gemini_model_ids:
                ...
                for enable_search in ([False, True] if want_search else [False]):
                    try:
                        ...
                        last_err = e
                        err = str(e)
                        logging.warning("Gemini key %s %s search=%s failed: %s", key_i + 1, model_id, enable_search, err[:300])
        ...
        for key_i, client in self._iter_gemini_clients():
            for model_id in self.gemini_model_ids:
                ...
                try:
                    ...
                    last_err = e
                    err = str(e)
                    logging.warning("Gemini stream key %s %s failed: %s", key_i + 1, model_id, err[:300])
                    ...
                    if action == "retry_same":
                        time.sleep(0.4)
                        continue
                    if action in ("retry_no_search", "next_model"):
                        continue
                    raise
        raise last_err or Exception("Gemini request failed")
