...
type Tab = "markets" | "backtest" | "strategies";
...
export default function TradePage() {
  ...
  const handleBacktest = async (params: BacktestParams, stayOnStrategies = false) => {
    if (!stayOnStrategies) setTab("backtest");
      ...
      <main className="max-w-[1720px] mx-auto px-4 lg:px-6 py-5 space-y-4">
        {online === false && (
          <div className="trade-panel flex items-start gap-3 px-4 py-3 text-[13px]" style={{ borderColor: "rgba(217, 119, 6, 0.3)", background: "rgba(217, 119, 6, 0.05)" }}>
            ...
            <span style={{ color: "var(--text-secondary)" }}>
              Crypto backend band hai. <code className="px-1.5 py-0.5 rounded" style={{ background: "var(--tr-field)", fontSize: 12 }}>cryptoproject</code>{" "}folder mein{" "}
              <code className="px-1.5 py-0.5 rounded" style={{ background: "var(--tr-field)", fontSize: 12 }}>python3 backend_api.py</code>{" "}chalao (port 2000).
            </span>
          </div>
        )}
          ...
          <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_368px] items-start">
            <section className="trade-panel overflow-hidden min-w-0">
                ...
                <div className="flex items-center gap-2">
                  <label className="flex items-center gap-2 text-[11px] font-semibold uppercase tracking-wider" style={{ color: "var(--text-muted)" }}>
                    ...
                    <input
                      ...
                      className="trade-input trade-w-bars trade-size-sm"
                    />
                  </label>
              ...
              >
                ...
                <StripCell label="Loaded bars" value={`${candles.length} · ${shortInterval(interval)}`} />
              </div>
        ...
        {tab === "strategies" && (
          <StrategyCards
            ...
            onBacktest={(params) => {
              setBacktestDefaults(params);
              void handleBacktest(params, true);
            }}
          />
        )}
      </main>
